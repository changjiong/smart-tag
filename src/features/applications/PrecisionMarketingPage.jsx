...
const PrecisionMarketingPage = () => {
  ...
  // 获取推荐渠道
  useEffect(() => {
    if (!selectedGoal || selectedGroups.length === 0 || selectedContents.length === 0) return;
    
    const fetchRecommendedChannels = async () => {
      setLoading(prev => ({ ...prev, channels: true }));
      try {
        const channels = await getChannelRecommendations(selectedGoal.id, selectedGroups);
  ...
  }, [selectedGoal, selectedGroups, selectedContents]);
  ...
  const handleGenerateStrategy = async () => {
    ...
    try {
      const result = await generateMarketingStrategy({
        ...
        targetGroups: selectedGroups.map(groupId => {
          const group = recommendedGroups.find(g => g.id === groupId);
          return {
            id: groupId,
            name: group ? group.name : '未知客群'
          };
        }),
        contents: selectedContents.map(contentId => {
          const content = recommendedContents.find(c => c.id === contentId);
          return {
            id: contentId,
            type: content ? content.type : '未知类型',
            title: content ? content.title : '未知内容'
          };
        }),
