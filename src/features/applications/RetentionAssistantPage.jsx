...
const RetentionAssistantPage = () => {
  ...
  // 获取渠道建议
  useEffect(() => {
    if (!selectedGroup || currentStep < 3) return;
    
    const fetchChannels = async () => {
      setLoading(prev => ({ ...prev, channels: true }));
      try {
        const channels = await getChannelRecommendations(selectedGroup.id);
  ...
  const handleGeneratePlan = async () => {
    ...
    try {
      const groupDetails = riskGroups.find(group => group.id === selectedGroup.id);
      const interventionDetails = selectedInterventions.map(id => {
        const intervention = interventions.find(item => item.id === id);
        return {
          id,
          name: intervention ? intervention.name : '未知干预措施',
      ...
      const channelDetails = selectedChannels.map(id => {
        ...
        return {
          id,
          name: channel ? channel.name : '未知渠道',
          type: channel ? channel.type : ''
        };
      });
      
      const result = await generateRetentionPlan({
        targetGroup: {
          id: selectedGroup.id,
          name: groupDetails ? groupDetails.name : '未知客群',
          size: groupDetails ? groupDetails.size : 0,
          churnRate: groupDetails ? groupDetails.churnRate : 0
        },
        selectedMetrics: selectedMetrics.map(id => {
          const metric = riskMetrics.find(m => m.id === id);
          return {
            id,
            name: metric ? metric.name : '未知指标'
          };
        }),
