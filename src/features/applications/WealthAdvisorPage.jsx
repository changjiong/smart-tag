...
const WealthAdvisorPage = () => {
  ...
  }, [assetAllocation, currentStep, investmentAmount, riskProfile, selectedGoal]);
  
  // 生成绩效预测和投资计划
  useEffect(() => {
    if (!productRecommendations || currentStep !== 4) return;
    
    const fetchPerformanceProjection = async () => {
      setLoading(prev => ({ ...prev, performance: true, plan: true }));
      try {
        // 1. 获取绩效预测
        const projection = await generatePerformanceProjection(
          selectedGoal.id,
          productRecommendations.map(p => p.id),
          investmentAmount
        );
        ...
        const plan = await generateWealthPlan({
          ...
          investmentAmount,
          allocationStrategy: assetAllocation,
  ...
  }, [productRecommendations, currentStep, selectedGoal, riskProfile, assetAllocation, investmentAmount]);
  ...
  const handleSavePlan = () => {
    message.success('投资方案已保存');
    // 这里可以添加保存逻辑
  };
  
  // 导出数据到数据输出功能
  const handleExportToDataOutput = () => {
    message.success('数据已导出到数据输出功能');
    // 这里添加导出逻辑
  };
